...
function OnClientCommandExecuted(sender, args) {
  if (args.get_commandName() == "SetImageProperties") {
    removeClearImgeDimensionsFunction();
  }
}

function removeClearImgeDimensionsFunction(count) {
  if (!count) count = 0;
  setTimeout(function () {
    try {
      $$('iframe[src^="Telerik.Web.UI.DialogHandler.aspx?DialogName=ImageProperties"]')[0].contentWindow.Telerik.Web.UI.Widgets.ImageProperties.prototype._clearImgeDimensions = function () {};
    } catch (e) {
      if (count < 10) {
        count++;
        removeClearImgeDimensionsFunction(count);
      }
    }
  }, 500);
}
